...
def plot_map(NAME, TSZ_map, CMB_KSZ_map):
    ...
    plt.savefig('results/sz_effect.png')
    return fig
...
def plot_w_hist(GLAT_slct, st_w, n_slct, style):
    ...
    with plt.style.context(style, after_reset=True):
        ...
        for i in range(6):
            ...
            plt.subplot(2,3,i+1)
            plt.hist(st_w[:n_slct,i], bins, facecolor=color[i])
            plt.title(str(freq[i])+'GHz')
        plt.savefig('results/w_hist.png')
    plt.show()
    return 0
...
def  plot_w_full_patch(w_full, moy_w_slct, moy_w, std_w_slct
    ...
    with plt.style.context(style, after_reset=True):
        ...
        plt.ylabel('Weight')
        plt.savefig('results/w_full.png')
    plt.show()
    return 0
