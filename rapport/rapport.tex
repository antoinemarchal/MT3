...
Une fois tours les profils de source récuppérés, il est possible de determiner un profil typique de sources
en fonction de leur rayon critique (cf Fig. \ref{profils_medians}). On peut voir en pointillé une estimation de 
la PSF : profil gaussien avec une largeur à mi-hauter déterminée à partir de la carte de planck 
avec la résolution la moin bonne. On voit sur cette figure que le profil médian correspondant à 
$r_c = 5 $ est en dessous de la PSF. \\
Une explication possible est qu'il y ait un exces de bruit positif à proximité de la source qui nous intéresse.
...
paramètres : off-set et largeur à mi-hauteur ) sur une section du profil radial que l'on sait n'etre
...
la gaussiene soient au niveau de  0  et refaire tourner l'algorithme qui determine le rayon critique
